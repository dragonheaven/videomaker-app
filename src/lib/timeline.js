...
const timelineEventsEmitter_1 = require('./timelineEventsEmitter');
const timelineUtils_1 = require('./utils/timelineUtils');
const timelineCursorType_1 = require('./enums/timelineCursorType');
const timelineKeyframeShape_1 = require('./enums/timelineKeyframeShape');
const timelineStyleUtils_1 = require('./utils/timelineStyleUtils');
const timelineElementType_1 = require('./enums/timelineElementType');
const timelineEvents_1 = require('./enums/timelineEvents');
const timelineCapShape_1 = require('./enums/timelineCapShape');
const timelineInteractionMode_1 = require('./enums/timelineInteractionMode');
const timelineSelectedEvent_1 = require('./utils/events/timelineSelectedEvent');
const timelineClickEvent_1 = require('./utils/events/timelineClickEvent');
const timelineDragEvent_1 = require('./utils/events/timelineDragEvent');
const defaults_1 = require('./settings/defaults');
const timelineEventSource_1 = require('./enums/timelineEventSource');
const timelineTimeChangedEvent_1 = require('./utils/events/timelineTimeChangedEvent');
const timelineSelectionMode_1 = require('./enums/timelineSelectionMode');
const timelineKeyframeChangedEvent_1 = require('./utils/events/timelineKeyframeChangedEvent');

const Timeline = /** @class */ (function (_super) {
  ...
  function Timeline(options, model) {
    ...
    _this._handleMouseDownEvent = function (args) {
      let isDoubleClick = Date.now() - _this._lastClickTime < _this._consts.doubleClickTimeoutMs;
      ...
      const event = new timelineClickEvent_1.TimelineClickEvent();
        ...
        if (isLeftClicked || isTouch) {
          if (_this._drag && !_this._startedDragWithCtrl) {
            ...
            if (_this._drag.type === timelineElementType_1.TimelineElementType.Timeline) {
              ...
              const movedOffset = _this._moveElements(offset, _this._drag.elements, timelineEventSource_1.TimelineEventSource.User);
              if (movedOffset !== 0) {
                if (!_this._drag.changed) {
                  _this._drag.prevVal = _this._drag.val;
                  const eventArgs = _this._emitDragStartedEvent();
    ...
    _this._handleMouseUpEvent = function (args) {
      if (_this._startPos) {
        ...
        const pos = _this._trackMousePos(_this._canvas, args);
        // Click detection.
        if (_this._clickAllowed || !_this._clickTimeoutIsOver() || (_this._drag && (_this._startedDragWithCtrl || _this._startedDragWithShiftKey))) {
          ...
          if (_this._interactionMode === timelineInteractionMode_1.TimelineInteractionMode.Zoom) {
            ...
            const selectionMode = args.shiftKey ? timelineSelectionMode_1.TimelineSelectionMode.Append : timelineSelectionMode_1.TimelineSelectionMode.Normal;
  ...
  Timeline.prototype._generateContainers = function (id) {
    ...
    this._canvas.style.cssText = 'image-rendering: -moz-crisp-edges;'
  ...
  Timeline.prototype._zoom = function (direction, speed, x) {
    if (speed > 0 && speed <= 1) {
      const deltaSpeed = timelineUtils_1.TimelineUtils.getDistance(this._width() / 2, x) * 0.2;
      x += deltaSpeed;
  ...
  Timeline.prototype._moveElements = function (offset, elements, source) {
    ...
    if (Math.abs(offset) > 0) {
      ...
      let bounds_1 = { min: Number.MIN_SAFE_INTEGER, max: Number.MAX_SAFE_INTEGER };
      ...
      elements.forEach((p) => {
        ...
        const currentBounds = timelineUtils_1.TimelineUtils.setMinMax(timelineUtils_1.TimelineUtils.setMinMax({ min: bounds_1.min, max: bounds_1.max }, p.keyframe), p.row);
        const expectedKeyframeValue = _this._options && _this._options.snapAllKeyframesOnMove ? _this.snapVal(p.keyframe.val) : p.keyframe.val;
        ...
        if (timelineUtils_1.TimelineUtils.isNumber(currentBounds.min) && newPosition < currentBounds.min) {
          ...
          offset += timelineUtils_1.TimelineUtils.getDistance(currentBounds.min, newPosition);
        }
        if (timelineUtils_1.TimelineUtils.isNumber(currentBounds.max) && newPosition > currentBounds.max) {
          // Return to the bounds:
          offset -= timelineUtils_1.TimelineUtils.getDistance(currentBounds.max, newPosition);
        }
      });
      if (Math.abs(offset) > 0) {
        ...
        elements.forEach((element) => {
          ...
          const newValue = _this._setKeyframePos(element, toSet, source);
  ...
  Timeline.prototype._performClick = function (pos, drag) {
    ...
    if (drag && drag.type === timelineElementType_1.TimelineElementType.Keyframe) {
      let mode = timelineSelectionMode_1.TimelineSelectionMode.Normal;
  ...
  Timeline.prototype._setKeyframePos = function (element, value, source) {
    ...
    if (element.keyframe && element.keyframe.val != value) {
      ...
      const event_1 = this._emitKeyframeChanged(element, source);
  ...
  Timeline.prototype._selectInternal = function (nodes, mode) {
    ...
    if (nodesArray && mode === timelineSelectionMode_1.TimelineSelectionMode.Append) {
      nodes.forEach((node) => {
        const changed = _this._changeNodeState(state, node, true);
  ...
  Timeline.prototype.snapVal = function (val) {
    ...
    if (this._options && this._options.snapEnabled && this._options.snapStep) {
      ...
      const minOffset = timelineUtils_1.TimelineUtils.sign(this._options.min) * (minSteps - Math.floor(minSteps)) * this._options.snapStep;
  ...
  Timeline.prototype._formatUnitsText = function (ms, isSeconds) {
    ...
    const sign = timelineUtils_1.TimelineUtils.sign(ms) < 0 ? '-' : '';
    ms = Math.abs(ms);
  ...
  Timeline.prototype._renderTicks = function () {
    ...
    const valDistance = timelineUtils_1.TimelineUtils.getDistance(from, to);
    if (valDistance <= 0) {
      return;
    }
    // Find the nearest 'beautiful' step for a gauge.
    // 'beautiful' step should be dividable by 1/2/5/10!
    const step = timelineUtils_1.TimelineUtils.findGoodStep(valDistance / (screenWidth / this._options.stepPx));
    const smallStep = timelineUtils_1.TimelineUtils.findGoodStep(valDistance / (screenWidth / this._options.stepSmallPx));
    ...
    let lastTextStart = 0;
    this._ctx.save();
    const headerHeight = timelineStyleUtils_1.TimelineStyleUtils.headerHeight(this._options);
  ...
  Timeline.prototype._calculateModel = function () {
    ...
    let rowAbsoluteHeight = timelineStyleUtils_1.TimelineStyleUtils.headerHeight(this._options);
    rows.forEach((row, index) => {
      if (!row || row.hidden) {
        return;
      }
      // draw with scroll virtualization:
      const rowHeight = timelineStyleUtils_1.TimelineStyleUtils.getRowHeight(row, _this._options);
      const marginBottom = timelineStyleUtils_1.TimelineStyleUtils.getRowMarginBottom(row, _this._options);
      const currentRowY = rowAbsoluteHeight - (_this._scrollContainer ? _this._scrollContainer.scrollTop : 0);
      ...
      const calcRow = {
        size: {
          x: 0, y: currentRowY, width: _this._canvas ? _this._width() : 0, height: rowHeight,
        },
        marginBottom,
        model: row,
        min: null,
      ...
      if (row && row.keyframes) {
        row.keyframes.forEach((keyframe) => {
          if (keyframe && !isNaN(keyframe.val) && !keyframe.hidden) {
            ...
            const keyframeSize = _this._getKeyframePosition(keyframe, calcRow);
            const calcKeyframe = {
              model: keyframe,
              parentRow: calcRow,
      ...
      calcRow.groups.forEach((group) => {
        ...
        const groupRect = _this._getKeyframesGroupSize(row, calcRow.size.y, group.min, group.max);
  ...
  Timeline.prototype._renderRows = function () {
    ...
    if (data && data.rows) {
      ...
      data.rows.forEach((rowCalc) => {
        ...
        const bounds = _this._cutBounds(rowCalc.size);
        if (bounds) {
          _this._ctx.fillRect(bounds.x, bounds.y, bounds.width, bounds.height);
        }
        const keyframeLaneColor = timelineStyleUtils_1.TimelineStyleUtils.groupFillColor(rowCalc.model, _this._options);
        if (!rowCalc.groups) {
          return;
        }
        rowCalc.groups.forEach((group) => {
          // get the bounds on a canvas
          const rectBounds = _this._cutBounds(group.size);
          if (rectBounds) {
  ...
  Timeline.prototype._cutBounds = function (rect) {
    ...
    const minX = 0; const maxX = this._width(); const minY = timelineStyleUtils_1.TimelineStyleUtils.headerHeight(this._options); const
      maxY = this._width();
  ...
  Timeline.prototype._getKeyframesGroupSize = function (row, rowY, minValue, maxValue) {
    let groupHeight = timelineStyleUtils_1.TimelineStyleUtils.rowGroupHeight(row, this._options);
    const height = timelineStyleUtils_1.TimelineStyleUtils.getRowHeight(row, this._options);
    ...
    const margin = height - groupHeight;
    // draw keyframes rows.
  ...
  Timeline.prototype._getKeyframePosition = function (keyframe, rowCalculated) {
    ...
    let height = timelineStyleUtils_1.TimelineStyleUtils.getKeyframeStyle(keyframe, rowCalculated.model, this._options, 'height', 'auto');
    let width = timelineStyleUtils_1.TimelineStyleUtils.getKeyframeStyle(keyframe, rowCalculated.model, this._options, 'width', 'auto');
    if (height == 'auto') {
  ...
  Timeline.prototype._renderKeyframes = function () {
    ...
    this._forEachKeyframe((calcKeyframe) => {
      ...
      if (pos) {
        let x = _this._getSharp(pos.x);
        let { y } = pos;
        const bounds = _this._cutBounds({
          x: x - pos.width / 2,
          y: y - pos.height / 2,
          width: pos.width,
          height: pos.height,
        ...
        const shape = timelineStyleUtils_1.TimelineStyleUtils.keyframeShape(keyframe, row, _this._options);
        if (shape === timelineKeyframeShape_1.TimelineKeyframeShape.None) {
          return;
        }
        const keyframeColor = keyframe.selected ? timelineStyleUtils_1.TimelineStyleUtils.keyframeSelectedFillColor(keyframe, row, _this._options) : timelineStyleUtils_1.TimelineStyleUtils.keyframeFillColor(keyframe, row, _this._options);
        const border = timelineStyleUtils_1.TimelineStyleUtils.keyframeStrokeThickness(keyframe, row, _this._options);
        const strokeColor = border > 0 ? (keyframe.selected ? timelineStyleUtils_1.TimelineStyleUtils.keyframeSelectedStrokeColor(keyframe, row, _this._options) : timelineStyleUtils_1.TimelineStyleUtils.keyframeStrokeColor(keyframe, row, _this._options)) : '';
  ...
  Timeline.prototype._setTimeInternal = function (val, source) {
    ...
    if (this._val != val) {
      ...
      const timelineEvent = new timelineTimeChangedEvent_1.TimelineTimeChangedEvent();
  ...
  Timeline.prototype.setTime = function (val) {
    ...
    const isChanged = this._setTimeInternal(val, timelineEventSource_1.TimelineEventSource.SetTimeMethod);
    if (isChanged) {
      this.rescale();
      this.redraw();
    }
  ...
  Timeline.prototype._getMousePos = function (canvas, e) {
    ...
    const scaleX = canvas.width / this._pixelRatio / rect.width; // relationship bitmap vs. element for X
  ...
  Timeline.prototype._findDraggable = function (elements, val) {
    ...
    const getPriority = function (type) {
      ...
      if (type === timelineElementType_1.TimelineElementType.Keyframe) {
        return 2;
      }
      if (type === timelineElementType_1.TimelineElementType.Group) {
        return 3;
      }
      return -1;
    };
  ...
  Timeline.prototype.elementFromPoint = function (pos, clickRadius) {
    ...
    if (pos.y >= headerHeight && this._options.keyframesDraggable) {
      this._forEachKeyframe((calcKeyframe, index, isNextRow) => {
        ...
        if (isNextRow) {
          const rowOverlapped = timelineUtils_1.TimelineUtils.isOverlap(pos.x, pos.y, calcKeyframe.parentRow.size);
          if (rowOverlapped) {
            const row = {
          ...
          if (calcKeyframe.parentRow.groups) {
            calcKeyframe.parentRow.groups.forEach((group) => {
              const keyframesGroupOverlapped = timelineUtils_1.TimelineUtils.isOverlap(pos.x, pos.y, group.size);
              if (keyframesGroupOverlapped) {
                const keyframesModels = _this._mapKeyframes(group.keyframes);
                const groupElement = {
                  val: _this._mousePosToVal(pos.x, snap),
                ...
                const snapped = _this.snapVal(group.min);
        ...
        if (keyframePos) {
          const dist = timelineUtils_1.TimelineUtils.getDistance(keyframePos.x, keyframePos.y, pos.x, pos.y);
  ...
  Timeline.prototype._mergeOptions = function (from) {
    ...
    const to = JSON.parse(JSON.stringify(defaults_1.defaultTimelineOptions));
  ...
  Timeline.prototype._emitKeyframeChanged = function (element, source) {
    if (source === void 0) { source = timelineEventSource_1.TimelineEventSource.Programmatically; }
    const args = new timelineKeyframeChangedEvent_1.TimelineKeyframeChangedEvent();
  ...
  Timeline.prototype._emitKeyframesSelected = function (state) {
    const args = new timelineSelectedEvent_1.TimelineSelectedEvent();
  ...
  Timeline.prototype._getDragEventArgs = function () {
    const draggableArguments = new timelineDragEvent_1.TimelineDragEvent();
    if (!this._drag) {
